...
- **Socket-safe recovery.** [`resumeChatGroupWatching`](src/lib/services/chatGroupWatch.svelte.ts:343) scopes a reconnect to the degraded coordinator instead of tearing down healthy subscriptions across all coordinators. Rather than aborting on the possibly-unhealthy old socket — relay publishes retry until ACKed, so that would hang indefinitely — a scoped resume locally discards that coordinator's watches ([`closeWatch`](src/lib/services/chatGroupWatch.svelte.ts:160) with `{ local: true }`) so the delta restart re-opens them. Resume never rebuilds the client eagerly: a transient stream error does not mean the socket is dead, and forcing a full relay reconnect + health reset on every blip made time-to-"Connected" scale with group count on mobile. [`withCoordinatorClient`](src/lib/services/chatRuntime.ts:226) rebuilds lazily (via [`replaceCoordinatorClient`](src/lib/services/chatRuntime.ts:237)) only when a coordinator call actually fails on the current client. Both scoped and unscoped resume converge on the same delta restart via [`startWatchingAllGroups`](src/lib/services/chatGroupWatch.svelte.ts:777): subscriptions that died while backgrounded unregister themselves on stream end, so only those groups are re-fetched and re-subscribed while healthy ones are left untouched. Account switches do not resume at all — they stop the previous account's watches and then call `startWatchingAllGroups`.
