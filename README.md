...
- **Scoped, socket-safe recovery.** [`resumeChatGroupWatching`](src/lib/services/chatGroupWatch.svelte.ts:369) scopes a reconnect to the degraded coordinator instead of tearing down healthy subscriptions across all coordinators. Rather than aborting on the possibly-unhealthy old socket — relay publishes retry until ACKed, so that would hang indefinitely — the resume path locally discards the watches and rebuilds the client via [`replaceCoordinatorClient`](src/lib/services/chatRuntime.ts:237) so re-subscribe uses a fresh socket. Account switches do not resume at all — they stop the previous account's watches and then call `startWatchingAllGroups`.
- **Liveness-gated rebuilds.** An unscoped resume (view change / online) probes each coordinator first via [`probeCoordinatorLiveness`](src/lib/services/chatGroupWatch.svelte.ts:589): a backlog fetch on the current client raced against a short timeout (which doubles as catch-up). Coordinators that respond keep their client and subscriptions; only unreachable ones are rebuilt via [`rebuildUnreachableCoordinators`](src/lib/services/chatGroupWatch.svelte.ts:355). Scoped resumes skip the probe and rebuild directly, since that coordinator is already known to be unreachable.
- **Silent fresh open.** The lifecycle listeners (`pageshow`/`focus`/`visibilitychange`) are gated on a `warmed` flag that is set only after the first watch startup completes, so a fresh app open does not churn the watches the steady-state layout `$effect` is already starting (and does not flash the reconnect banner). The `online` listener stays unconditional since it signals genuine connectivity recovery.
