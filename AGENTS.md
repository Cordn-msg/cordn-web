...
- Group message ingestion is centralized in [`applyIncomingChatGroupMessages()`](src/lib/services/chatGroups.svelte.ts:1294) so watch-driver backlog fetches and live subscriptions via [`ingestIncomingChatGroupMessages()`](src/lib/services/chatGroups.svelte.ts:1392) stay consistent.
- Active group watching is owned by [`startWatchingAllGroups()`](src/lib/services/chatGroupWatch.svelte.ts:900), the single re-entrancy-safe entry point used by both the account-change handler and the chat-layout `$effect`. It must remain the default path for keeping groups current instead of adding parallel fetch-heavy flows or a second competing watch driver.
...
- The news feed ([`src/routes/chat/news`](src/routes/chat/news/+page.svelte)) is global, build-time, read-only content authored in [`newsReleases`](src/lib/news/feedItems.ts:83). Unread state is a per-release version map, not a global timestamp: each [`NewsRelease`](src/lib/news/feedItems.ts:17) carries a `version` (default 1), and a release is unread when its version is higher than what the reader's `cordn.newsReadVersions` localStorage map has seen. To re-notify readers of an already-published release (including multiple same-day edits), bump only its `version` and leave `createdAt` untouched — `createdAt` is the day label, `version` is the "this changed" signal. The feed sorts ascending by `createdAt` (oldest at top, newest at the bottom, like chat messages), so authors can append in any order; within a day, array order is preserved. Use a Markdown bullet list (`- `) to accumulate multiple same-day changes as dots in one card. Each bullet starts with one of three emojis to signal intent: ✨ for new features, 🐛 for fixes, 🔄 for changes and improvements. Donation items never count toward the badge.

## Versioning

- The single source of truth for the product version is [`package.json`](package.json) `version` (semver). Cut releases with `pnpm version` (e.g. `pnpm version prerelease --preid=next`); never hand-edit the version field. The full lifecycle is in [`docs/versioning.md`](docs/versioning.md).
- `pnpm version` runs the `version` lifecycle hook → [`scripts/changelog.mjs`](scripts/changelog.mjs) regenerates [`CHANGELOG.md`](CHANGELOG.md) from Conventional Commits since the previous tag and stages it into the release commit. **Write Conventional Commit messages** (`feat:`, `fix:`, `docs:`, `chore:`, optional `(scope)`) — non-conventional messages won't appear in the changelog. Don't hand-edit `CHANGELOG.md`.
- Two distinct version tokens coexist on purpose — do not collapse them (details in [`docs/versioning.md`](docs/versioning.md)):
  - `__APP_VERSION__` (git SHA) is the **per-build deploy-change token** driving the web update banner in [`appUpdate.svelte.ts`](src/lib/services/appUpdate.svelte.ts). It must change on every build; semver is too coarse (a hotfix rebuild keeps the same semver). Never replace it with semver.
  - `__APP_SEMVER__` (package.json version) is for **user-facing display** (toast, banner). Both are written to [`static/version.json`](static/version.json) by [`scripts/write-version.mjs`](scripts/write-version.mjs) and stamped by [`vite.config.ts`](vite.config.ts).
- Android versioning derives from the same source in [`android/app/build.gradle`](android/app/build.gradle): `versionName` ← package.json `version`, `versionCode` ← `git rev-list --count HEAD` (monotonic across prereleases). Don't hardcode either.
- Native and web update through different mechanisms and must not be coupled: **web** updates on every deploy (SHA banner → reload); **native** updates only via a new APK build + redistribute (assets are bundled in the APK, the service worker is dormant, no OTA path by design). Don't introduce live-update plugins or `server.url` remote loading.
- Changesets was evaluated and rejected: stable `@changesets/cli` bundles `@manypkg/get-packages`, which can't see the workspace root. `pnpm version` is the native fit for this single-app repo (see [`docs/versioning.md`](docs/versioning.md)).
...
- Remote reads currently managed through Svelte Query:
  ...
  - **Welcome notifications** — fetched via [`welcomeNotificationsQueryOptions()`](src/lib/queries/chatWelcomeQueries.ts:51) with `refetchInterval`-based polling. The coordinator now uses TTL-based non-destructive welcome storage, so accepted and dismissed welcomes are retained locally to prevent reappearance across fetches.
...
- All fetch/subscribe call sites pass [`since_epoch`](src/lib/contracts/index.ts:112) — derived from [`joinEpoch`](src/lib/services/chatGroups.svelte.ts:112) via [`toWatchableGroup`](src/lib/services/chatGroupWatch.svelte.ts:315) — so the coordinator filters out pre-join messages that are undecryptable. The group creator defaults to `0n` (no filter). When touching fetch/subscribe logic in [`chatGroupWatch.svelte.ts`](src/lib/services/chatGroupWatch.svelte.ts), preserve `sinceEpoch` in every request path.
- Invalidation rules:
  - After publishing, removing, or consuming a key package, invalidate the matching `available-key-packages` query key.
  - After account change, remove all queries scoped to the previous account via [`queryClient.removeQueries()`](src/lib/services/chatGroupWatch.svelte.ts:119).
